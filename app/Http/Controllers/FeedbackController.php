...
class FeedbackController extends Controller
{
    ...
    {
        $registrationType = $type;  // $type will be passed as 'existing' or 'new'
        ...
        $newUserRules = [
            ...
            'citizenship_district' => 'required|string|max:100',
            'photo' => 'nullable|file|mimes:jpg,jpeg,png|max:2048',
            'citizenship' => $request->is_minor
                ? (isset($user) && $user->citizenship ? 'nullable|file|mimes:jpg,jpeg,png|max:2048' : 'required|file|mimes:jpg,jpeg,png|max:2048')
            ...
            'parent_ward_no' => 'required_if:is_minor,1',
            'parent_tole' => 'required_if:is_minor,1',
            'parent_citizenship' => $request->is_minor == 1 && !$user->parent_citizenship
                ? 'required|mimes:jpeg,png,jpg|max:2048'
                : 'nullable|mimes:jpeg,png,jpg|max:2048',
            'minor_guardian_relation' => 'required_if:is_minor,1',
            'minor_guardian_bank_name' => 'required_if:is_minor,1',
            'minor_guardian_bank_account_number' => 'required_if:is_minor,1',
            'national_id_no' => $request->is_minor ? 'nullable|string' : 'required|string',
            'national_id' => $request->is_minor
                ? (isset($user) && $user->national_id ? 'nullable|file|mimes:jpg,jpeg,png|max:2048' : 'required|file|mimes:jpg,jpeg,png|max:2048')
                : 'required|file|mimes:jpg,jpeg,png|max:2048',
            'birth_certificate' => $request->is_minor == 1 && !$user->birth_certificate
                ? 'required|mimes:jpeg,png,jpg|max:2048'
                : 'nullable|mimes:jpeg,png,jpg|max:2048',
            'share_initial' => 'required|numeric',
            'investment_amount_initial' => 'nullable|numeric',
        ...
        $commonRules = [
            ...
            'accept_terms' => 'accepted',
            'signature' => 'nullable|file|mimes:jpg,jpeg,png|max:2048',
            'voucher' => $request->payment_method == 'cheque' && !$user->voucher
                ? 'required|mimes:jpeg,png,jpg,pdf|max:2048'
                : 'nullable|mimes:jpeg,png,jpg,pdf|max:2048',
            'bank_name' => 'required_if:payment_method,cheque',
            'bank_branch' => 'required_if:payment_method,cheque',
            'account_holder_name' => 'required_if:payment_method,cheque',
            'account_number' => 'required_if:payment_method,cheque',
        ...
        try {
            ...
            // Update the registration number for 'new' users
            if ($registrationType === 'new') {
                $validated['registration_number'] = 'temporary' . uniqid();
            } else {
                $validated['registration_number'] = $request->input('registration_number');
            }
            ...
            // Update or create investment details if needed
            $investmentDetailsData = [
                'registration_number' => $validated['registration_number'],
                'share' => $validated['share'],
                'amount_in_words' => $validated['amount_in_words'],
            ...
            $user->investmentDetails()->update(
                ['new_user_id' => $user->id],
                $investmentDetailsData
            );

            return redirect()->route('admin.edit', ['id' => $user->id, 'type' => $registrationType])
